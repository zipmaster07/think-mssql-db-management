...
SET @printMessage = 'Setting up dbAdmin database for dev instance:' + char(13) + char(10);
	...
	*/
	BEGIN TRAN insertStmts
		...
		BEGIN TRY
			...
			INSERT INTO params (p_key, p_value)
				VALUES ('DefaultBackupDirectory', 'U:\MSSQL10_50.DEV\MSSQL\Backup\');
		...
		BEGIN TRY
			...
			INSERT INTO params (p_key, p_value)
				VALUES ('DefaultDataDirectory', 'R:\MSSQL10_50.DEV\MSSQL\Data\');
		...
		BEGIN TRY
			...
			INSERT INTO params (p_key, p_value)
				VALUES ('DefaultLogDirectory', 'S:\MSSQL10_50.DEV\MSSQL\Logs\');
		...
		IF NOT EXISTS (SELECT 1 FROM user_mappings WHERE user_name = 'brichan')
		BEGIN TRY

			INSERT INTO user_mappings (domain_name, user_name, dept_name)
				VALUES ('MPLS\brichan','brichan','dev');

			RAISERROR ('	Values added successfully: brichan', 10, 1) WITH NOWAIT;
		END TRY
		BEGIN CATCH
		...
		END CATCH;
		ELSE
			RAISERROR ('	Values "brichan" already inserted, skipping...', 10, 1) WITH NOWAIT;

		IF NOT EXISTS (SELECT 1 FROM user_mappings WHERE user_name = 'rwilson')
		BEGIN TRY

			INSERT INTO user_mappings (domain_name, user_name, dept_name)
				VALUES ('MPLS\rwilson','rwilson','dev');

			RAISERROR ('	Values added successfully: rwilson', 10, 1) WITH NOWAIT;
		END TRY
		BEGIN CATCH
		...
		END CATCH;
		ELSE
			RAISERROR ('	Values "rwilson" already inserted, skipping...', 10, 1) WITH NOWAIT;

		IF NOT EXISTS (SELECT 1 FROM user_mappings WHERE user_name = 'jsherrill')
		BEGIN TRY

			INSERT INTO user_mappings (domain_name, user_name, dept_name)
				VALUES ('MPLS\jsherrill','jsherrill','dev');

			RAISERROR ('	Values added successfully: jsherrill', 10, 1) WITH NOWAIT;
		END TRY
		BEGIN CATCH
		...
		END CATCH;
		ELSE
			RAISERROR ('	Values "jsherrill" already inserted, skipping...', 10, 1) WITH NOWAIT;
		...
		SET @printMessage = 'Done inserting into baseline_versions' + char(13) + char(10) + char(13) + char(10) + 'dbAdmin database is now setup for the dev instance'
		RAISERROR(@printMessage, 10, 1) WITH NOWAIT;
	COMMIT TRAN;
END TRY
BEGIN CATCH
