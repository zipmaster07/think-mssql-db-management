...
SET @printMessage = 'Setting up dbAdmin database for support instance:' + char(13) + char(10);
	...
	*/
	BEGIN TRAN insertStmts
		...
		BEGIN TRY
			...
			INSERT INTO params (p_key, p_value)
				VALUES ('DefaultBackupDirectory', 'U:\MSSQL10_50.SUPPORT\MSSQL\Backup');
		...
		BEGIN TRY
			...
			INSERT INTO params (p_key, p_value)
				VALUES ('DefaultDataDirectory', 'R:\MSSQL10_50.SUPPORT\MSSQL\Data\');
		...
		BEGIN TRY
			...
			INSERT INTO params (p_key, p_value)
				VALUES ('DefaultLogDirectory', 'S:\MSSQL10_50.SUPPORT\MSSQL\Logs\');
		...
		IF NOT EXISTS (SELECT 1 FROM user_mappings WHERE user_name = 'kturpin')
		BEGIN TRY

			INSERT INTO user_mappings (domain_name, user_name, dept_name)
				VALUES ('MPLS\kturpin','kturpin','support');

			RAISERROR ('	Values added successfully: kturpin', 10, 1) WITH NOWAIT;
		END TRY
		BEGIN CATCH
		...
		END CATCH;
		ELSE
			RAISERROR ('	Values "kturpin" already inserted, skipping...', 10, 1) WITH NOWAIT;
		...
		IF NOT EXISTS (SELECT 1 FROM user_mappings WHERE user_name = 'trsmith')
		BEGIN TRY

			INSERT INTO user_mappings (domain_name, user_name, dept_name)
				VALUES ('MPLS\trsmith','trsmith','support');

			RAISERROR ('	Values added successfully: trsmith', 10, 1) WITH NOWAIT;
		END TRY
		BEGIN CATCH
		...
		END CATCH;
		ELSE
			RAISERROR ('	Values "trsmith" already inserted, skipping...', 10, 1) WITH NOWAIT;

		IF NOT EXISTS (SELECT 1 FROM user_mappings WHERE user_name = 'bjensen')
		BEGIN TRY

			INSERT INTO user_mappings (domain_name, user_name, dept_name)
				VALUES ('MPLS\bjensen','bjensen','impl');

			RAISERROR ('	Values added successfully: bjensen', 10, 1) WITH NOWAIT;
		END TRY
		BEGIN CATCH
		...
		END CATCH;
		ELSE
			RAISERROR ('	Values "bjensen" already inserted, skipping...', 10, 1) WITH NOWAIT;

		IF NOT EXISTS (SELECT 1 FROM user_mappings WHERE user_name = 'rsigman')
		BEGIN TRY

			INSERT INTO user_mappings (domain_name, user_name, dept_name)
				VALUES ('MPLS\rsigman','rsigman','impl');

			RAISERROR ('	Values added successfully: rsigman', 10, 1) WITH NOWAIT;
		END TRY
		BEGIN CATCH
		...
		END CATCH;
		ELSE
			RAISERROR ('	Values "rsigman" already inserted, skipping...', 10, 1) WITH NOWAIT;
		...
		SET @printMessage = 'Done inserting into baseline_versions' + char(13) + char(10) + char(13) + char(10) + 'dbAdmin database is now setup for the support instance'
		RAISERROR(@printMessage, 10, 1) WITH NOWAIT;
	COMMIT TRAN;
END TRY
BEGIN CATCH
