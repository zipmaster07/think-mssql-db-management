...
CREATE PROCEDURE [dbo].[adm_addBaseline] (
	@baselineFile		nvarchar(512) = null	--Optional:	The filename, including path, of the baseline database that is to be added. This is required if adding an available baseline version.
	...
	,@thkVersion		nvarchar(16) = null		--Optional:	Version of the restored database without dots. This is required if adding an unavailable baseline version.
...
DECLARE @newBaselineDb				sysname = 'newBaselineDb'	--The name of the temporary database that will be used to stage the baseline DB.
		...
		,@definedTempTableId		int = 1						--Used to ID all temp tables.  This is used so that multiple restores can take place at once (a.k.a. multiple people can run the stored procedure at once).
...
BEGIN
	...
	IF @baselineAvailable = 1 AND @baselineFile is NULL
	BEGIN TRY
		RAISERROR(93106, -1, -1)
	END TRY
	BEGIN CATCH
		EXEC dbo.sub_formatErrorMsg @formatSpName = 'dbo.adm_addBaseline'
			,@errorSeverity = @severity OUTPUT
			,@formatDebug = @baselineDebug
		IF @severity >= 16
			RETURN -1
	END CATCH
	
	IF @baselineAvailable = 0 AND @thkVersion is NULL
	BEGIN TRY
		RAISERROR(93107, -1, -1)
	END TRY
	BEGIN CATCH
...
BEGIN
	...
	EXEC sp_executesql @sql, N'@thkVersionIN nvarchar(16) OUTPUT', @thkVersion OUTPUT; --Finds the THINK Enterprise version of the restored database.
...
BEGIN
	...
	SET @filename = 'baseline_' + @mssqlVersion + REPLACE(@thkVersion, '.', '') + N'.bak';
	...
	INSERT INTO dbAdmin.dbo.baseline_versions (baseline_version, major_version_order, minor_version_order, patch_version_order, backup_path, is_available)
		VALUES (@thkVersion, @majorVersionOrder, @minorVersionOrder, @patchVersionOrder, @backupLocation, @baselineAvailable)

	IF @baselineAvailable = 1
		EXEC dbo.adm_deleteDatabase @deleteDbName = @newBaselineDb
				,@deleteBackupHist = 1
				,@deleteDebug = @baselineDebug;
END;