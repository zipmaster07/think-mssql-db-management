...
**	
**	There are many other features of this sp.  Documentation for is kept at "\\brighton\Public\Customer Service\Projects\SQL Server Operations\Database Management\Documentation".
**	
...
DECLARE @restoreStartTime		datetime		--Date and time the database restore started.
		...
		,@daysToLive			int				--How many days the backup should be kept.  Only used when @cleanOverride is not set to null.  When set to "clean", the backup is kept indefinitely.  When set to "dirty", the backup is kept for 60 days (shorter than default).
...
SET @sql =
	CASE
		WHEN @@SERVICENAME = 'LEGACY' --If running under the LEGACY instance add the listed MPLS accounts.
			THEN N'GRANT EXECUTE ON [dbo].[usp_THKRestoreDB] TO [MPLS\akennedy]
					GRANT EXECUTE ON [dbo].[usp_THKRestoreDB] TO [MPLS\bjensen]
