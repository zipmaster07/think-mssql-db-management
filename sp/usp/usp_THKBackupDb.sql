...
CREATE PROCEDURE [dbo].[usp_THKBackupDB](
	...
	,@setBackupPath			nvarchar(16) = null			--Optional:		Which location the database should be backed up to: Default, Customer First, DB Changes, Baseline.
	...
	,@setDebug				nchar(1) = 'n'				--Optional:		Returns additional debugging messages.
...
BEGIN TRY
	...
	*/
	BEGIN
		...
		IF @setBackupPath not in (NULL, 'default', 'customer first', 'db changes', 'baseline')
			RAISERROR('Value for parameter @setBackupPath must be "default", "customer first", "db changes", "baseline" or must not be specified.', 16, 1) WITH LOG;
					...
					*/
					...
					SET @litespeedFilePath =
						CASE @setBackupPath
							...
							WHEn 'baseline'
								THEN (SELECT p_value FROM [dbAdmin].[dbo].[params] WHERE p_key = 'BaselineBackupDirectory')
						END;
					...
					END TRY
					BEGIN CATCH
						EXEC dbo.sub_formatErrorMsg @formatSpName = 'dbo.usp_THKBackupDb'
							,@errorSeverity = @severity OUTPUT
							,@formatDebug = @setDebug;
						IF @severity >= 16
							RETURN -1
					END CATCH;
	...
	*/
	BEGIN
		...
		SET @sql = N'IF EXISTS (SELECT 1 FROM ' + @setDbName + '.INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME =  ''config'')
													SET @thkVersionIN = (SELECT cur_vers FROM ' + @setDbName + N'.dbo.config)
												ELSE
													SET @thkVersionIN = NULL';
		EXEC sp_executesql @sql, N'@thkVersionIN nvarchar(32) OUTPUT', @thkVersion OUTPUT; --Finds if the database is a THINK Enterprise DB and if so then finds the version of that database.
...
SET @sql =
	CASE
		...
		WHEN @@SERVICENAME = 'DEV' --If running under the DEV instance add the listed MPLS accounts.
			THEN N'GRANT EXECUTE ON [dbo].[usp_THKBackupDB] TO [MPLS\mheil]
		...
		WHEN @@SERVICENAME = 'SQL11' --If running under the SQL11 instance add the listed MPLS accounts.
			THEN N'GRANT EXECUTE ON [dbo].[usp_THKBackupDB] TO [MPLS\cjenkins]
					GRANT EXECUTE ON [dbo].[usp_THKBackupDB] TO [MPLS\jschaeffer]
					GRANT EXECUTE ON [dbo].[usp_THKBackupDB] TO [MPLS\lzibetti]
