...
SET @printMessage = 'Setting up dbAdmin database for qa instance:' + char(13) + char(10);
	...
	*/
	BEGIN TRAN insertStmts
		...
		BEGIN TRY
			...
			INSERT INTO db_stored_proc_rights (stored_proc) --Insert into table
		...
		BEGIN TRY
			...
			INSERT INTO db_stored_proc_rights (stored_proc) --Insert Statement
		...
		BEGIN TRY
			...
			INSERT INTO params (p_key, p_value)
				VALUES ('DefaultBackupDirectory', 'U:\MSSQL10_50.QA\MSSQL\Backup\');
		...
		BEGIN TRY
			...
			INSERT INTO params (p_key, p_value)
				VALUES ('DefaultDataDirectory', 'R:\MSSQL10_50.QA\MSSQL\Data\');
		...
		BEGIN TRY
			...
			INSERT INTO params (p_key, p_value)
				VALUES ('DefaultLogDirectory', 'S:\MSSQL10_50.QA\MSSQL\Logs\');
		...
		IF NOT EXISTS (SELECT 1 FROM user_mappings WHERE user_name = 'jdennison')
		BEGIN TRY
	
			INSERT INTO user_mappings (domain_name, user_name, dept_name)
				VALUES ('MPLS\jdennison','jdennison','qa');

			RAISERROR ('	Values added successfully: jdennison', 10, 1) WITH NOWAIT;
		END TRY
		BEGIN CATCH
		...
		END CATCH;
		ELSE
			RAISERROR ('	Values "jdennison" already inserted, skipping...', 10, 1) WITH NOWAIT;
		...
		SET @printMessage = 'Done inserting into baseline_versions' + char(13) + char(10) + char(13) + char(10) + 'dbAdmin database is now setup for the qa instance'
		RAISERROR(@printMessage, 10, 1) WITH NOWAIT;
	COMMIT TRAN;
END TRY
BEGIN CATCH
