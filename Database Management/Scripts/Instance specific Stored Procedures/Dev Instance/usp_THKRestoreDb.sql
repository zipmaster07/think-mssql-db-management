...
CREATE PROCEDURE [dbo].[usp_THKRestoreDB](
	...
	,@setProbNbr	nvarchar(16) = null		--Undocumented:	Only used when restoring a dirty database.  Used for auditing
...
DECLARE @restoreStartTime		datetime		--Date and time the database restore started
		...
		,@definedTempTableId	int				--Used to ID the temp cursor_principals table.  This is used so that multiple restores can take place at once
...
BEGIN TRY
	...
	**	it is NULL) then we are assuming that the database is already clean or we are (which is acceptable)
...
GRANT EXECUTE ON [dbo].[usp_THKRestoreDB] TO [MPLS\brichan]
GRANT EXECUTE ON [dbo].[usp_THKRestoreDB] TO [MPLS\rwilson]
GRANT EXECUTE ON [dbo].[usp_THKRestoreDB] TO [MPLS\jsherrill]
GO
*/