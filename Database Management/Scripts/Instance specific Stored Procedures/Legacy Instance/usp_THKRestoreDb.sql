...
DECLARE @restoreStartTime		datetime		--Date and time the database restore started
		...
		,@daysToLive			int				--How many days the backup should be kept.  Only used when @cleanOverride is not set to null.  When set to yes, the backup is kept indefinitely.  When set to no, the backup is kept for 60 days (shorter than default)
...
GRANT EXECUTE ON [dbo].[usp_THKRestoreDB] TO [MPLS\kturpin]
GRANT EXECUTE ON [dbo].[usp_THKRestoreDB] TO [MPLS\lzibetti]
GRANT EXECUTE ON [dbo].[usp_THKRestoreDB] TO [MPLS\rsigman]
GRANT EXECUTE ON [dbo].[usp_THKRestoreDB] TO [MPLS\trsmith]
GO
*/